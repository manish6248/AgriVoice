...
def test_send_audio():
    """
    Test sending audio files to farmers
    """
    # Configuration
    ...
    for farmer in farmers:
        ...
        for i, audio in enumerate(recent_files):
            ...
            media_url = f"{server_url}/static/audio/{audio['filename']}"
        ...
        time.sleep(1)  # Small delay between farmers
    
    logging.info(f"Finished sending audio files to {len(farmers)} farmers")

if __name__ == "__main__":
    print("Starting audio file test...")
    test_send_audio()
